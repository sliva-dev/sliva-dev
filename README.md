# 👋 Привет! Я Sliva

**Fullstack Infrastructure Engineer**

Железо, системы, автоматизация, код. Комплексные IT-решения под ключ: **Backend, Frontend, DevOps, Системное администрирование** — всё в одном флаконе.

> "Если это работает — значит, я мог бы сделать ещё лучше 🚀"

---

## 🛠️ Мои навыки

### Языки:

![Python](https://img.shields.io/badge/Python-3.11%2B-3776AB?logo=python\&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-ES6-F7DF1E?logo=javascript\&logoColor=black)
![HTML5](https://img.shields.io/badge/HTML5-E34F26?logo=html5\&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?logo=css3\&logoColor=white)

### Инструменты и технологии:

![Docker](https://img.shields.io/badge/Docker-2496ED?logo=docker\&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?logo=git\&logoColor=white)
![CI/CD](https://img.shields.io/badge/CI/CD-00BFFF?logo=gitlab\&logoColor=white)
![Облако](https://img.shields.io/badge/PrivateCloud-6e5494?logo=cloudflare\&logoColor=white)

---

## 🚀 Проекты

### 🤖 Proxmox VE Telegram Bot

[![Python 3.11+](https://img.shields.io/badge/Python-3.11%2B-3776AB.svg?logo=python\&logoColor=white)](https://python.org)
[![python-telegram-bot](https://img.shields.io/badge/telegram--bot-v22.5-2CA5E0.svg?logo=telegram)](https://python-telegram-bot.org)
[![Proxmox VE](https://img.shields.io/badge/Proxmox-8.x%2B-EC6601.svg?logo=proxmox)](https://proxmox.com)
[![Version](https://img.shields.io/badge/Version-1.3-blue.svg)](https://github.com/sliva/proxmox-telegram-bot)

> **Самый продвинутый и безопасный Telegram-бот для управления Proxmox VE в 2025 году**
> Всё, что нужно системному администратору: мониторинг, алерты, управление VM/LXC и безопасный шелл — прямо в чате.

[Смотреть на GitHub](https://github.com/sliva-dev/proxmox-telegram-bot)

###🤖 Discord Bot

[![Python 3.10+](https://img.shields.io/badge/Python-3.10%2B-3776AB.svg?logo=python&logoColor=white)](https://python.org)
[![Disnake](https://img.shields.io/badge/Disnake-2.9%2B-5865F2.svg?logo=discord)](https://docs.disnake.dev)
[![Architecture](https://img.shields.io/badge/Architecture-Modular-blue.svg)](https://github.com)
[![Version](https://img.shields.io/badge/Version-1.0-blue.svg)](https://github.com/sliva/proxmox-telegram-bot)

> **Самый функциональный Discord бот в 2025 году**
> Всё, что нужно владельцам серверов: тикеты, модерирование и логи.

[Смотреть на GitHub](https://github.com/sliva-dev/discord-bot)

---

## 📫 Связаться со мной

[![Telegram](https://img.shields.io/badge/Telegram-@shizaterrorblade-blue?style=for-the-badge\&logo=telegram\&logoColor=white)](https://t.me/shizaterrorblade)
[![Email](https://img.shields.io/badge/Email-sliva@sliva.su-red?style=for-the-badge\&logo=gmail\&logoColor=white)](mailto:sliva@sliva.su)
[![Portfolio](https://img.shields.io/badge/Portfolio-sliva.su-green?style=for-the-badge)](https://sliva.su)

---

## 🌟 Обо мне

* Люблю автоматизировать всё, что двигается (и даже то, что не двигается 😎)
* Верю, что хороший инженер — это тот, кто умеет делать backend, frontend и инфраструктуру сразу
* Работаю с комплексными системами, проектирую решения от железа до кода
* Постоянно учусь и внедряю новые технологии, чтобы быть на гребне инноваций

---

<div align="center">
<p>Если проект понравился — ставьте ⭐ на GitHub!</p>
</div>
